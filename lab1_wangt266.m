...
clear x; %Clear the x array
N=5000001; 
h = (5)/(N-1); 
x = zeros(N,1); %Preallocate the array

for j = 1:N
    x(j) = (j-1) * h-5; %Allocate array
end
...
% The general solution is f(t)=(t+c)/(e^t^2)
% If y(1)=-1, c = -1-e
% If y(1)=0, c = -1
% If y(1)=1, c = e-1

t = linspace(1,5, 100);    %Create the array

y=(t-1-exp(1))./exp(t.^2);   %First solution

yy=(t-1)./exp(t.^2);   %Second solution

yyy=(t-1+exp(1))./exp(t.^2);   %Third solution

plot(t,y,t,yy,t,yyy)

xlabel('x');
ylabel('f(x)');
title('Three solutions to the ODE with different inital conditions');
legend('y(1) = -1', 'y(1) = 0','y(1) = 1','Location','SouthWest');
...
% Inline function
f = @(x) (x.^3 - abs(x)) .* log10(x.^2+1)/log10(exp(1));
...
% Construct an array of 100 grid points from -5 to 5, and plot f(t).
t = linspace(-5,5,100);
plot(t, f(t));
% Label axes
xlabel('t');
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %define function

x = 0; y = 0; %finding values
fprintf(' f(x = %g, y = %g) = %g\n', x, y, f(x, y));
x = pi/2; y = 1;
fprintf(' f(x = %g, y = %g) = %g\n', x, y, f(x, y));
x = -1; y = -1;
...
guess = 3; %computing the roots
y = fzero(@(y) f(0, y), guess);
fprintf(' y(%g) = %g\n', 0, y);

guess = 0.5;
x = fzero(@(x) f(x, 0.5), guess);
fprintf(' y(%g) = %g\n', x, 0.5);
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4; %defining the funciton

xvals = linspace(-2, 2, 100); %creating the points

yvals = zeros(size(xvals));

for j = 1:length(xvals) %setting the values
   
   yvals(j) = fzero(@(y) f(xvals(j), y), 0);
   
end

plot(xvals, yvals); %plotting and labelling
xlabel('x');
ylabel('y');
...
% General Solution : y=e^(-sinx + x^3 + C)
% Particular Solution: y=e^(-sinx + x^3)
...
% The solution looks like a horizontal line and concentric U shapes above
% and below the line at y=0. 

% As X approaches infinity, at y=0 dy/dx is still 0, but the slopes of
% solutions not at y=0 become steeper and steeper. 

% The solutions are symmetric about the y-axis because xy^2 is even,
% meaning that the answer for xy^2 and x(-y)^2 are the same. 
...
% The constant solutions that are approached are y=1, y=2 and y=-1. y=2 is
% unstable, y=1 is stable, and y=-1 is semi-stable. 
